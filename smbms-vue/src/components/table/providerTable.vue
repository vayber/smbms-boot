...
<script>
	import {getProviderCount,getProviderList,getProviderPageList,deleteProvider} from "../../network/data/provider.js"
	export default{
		name:"providerTable",
		activated() {
			...
			setTimeout(()=>{
			getProviderPageList(this.providerCode,this.providerName,1,this.pageSize).then(res =>{
		...
		methods:{
			...
			currentChange(currentPage){
				getProviderPageList(this.providerCode,this.providerName,currentPage,this.pageSize).then(res =>{
				 this.providerList = res
				})
			}
